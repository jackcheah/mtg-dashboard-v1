...
def simulate_round_results(tournament, round_num):
    """Simulate random results for a round and submit them.

    This triggers the generation of the next Swiss round based on scores.
    Scoring: Winner gets 5 points, losers get 0, draw gives 1 point each.
    """
    if round_num not in tournament.tables:
        print(f"    Round {round_num} not in tables - skipping")
        return
    ...
    # Simulate results for each table
    for table_id, table_data in tables.items():
        if isinstance(table_data, dict):
            players = table_data.get('players', [])
        elif isinstance(table_data, list):
        ...
        # Randomly decide: 80% chance of a winner, 20% chance of draw
        if random.random() < 0.8 and len(players) >= 1:
            # Pick a random winner
            winner_idx = random.randint(0, len(players) - 1)
            for i, player in enumerate(players):
                player_id = player.get('Player ID') if isinstance(player, dict) else None
                if player_id:
                    if i == winner_idx:
                        tournament.player_scores[player_id] = tournament.player_scores.get(player_id, 0) + 5
                    else:
                        tournament.player_scores[player_id] = tournament.player_scores.get(player_id, 0) + 0
        else:
            # Draw - everyone gets 1 point
            for player in players:
                player_id = player.get('Player ID') if isinstance(player, dict) else None
                if player_id:
                    tournament.player_scores[player_id] = tournament.player_scores.get(player_id, 0) + 1

        tournament.round_results[round_num]['table_submissions'][table_id] = True
    ...
    # Generate next Swiss round if applicable
    last_swiss_round = tournament.swiss_rounds_count
    if round_num < last_swiss_round:
        next_round = round_num + 1
        print(f"    Generating Swiss Round {next_round} based on standings...")
        success = tournament.generate_swiss_round(next_round)
        if success:
            print(f"    ✓ Round {next_round} generated")
        else:
            print(f"    ✗ Failed to generate Round {next_round}")
    elif round_num == last_swiss_round:
        # End of Swiss - generate playoffs
        tournament.swiss_round_scores = tournament.scores.copy()
        if tournament.has_semifinals:
            print(f"    Generating Top 8 Cut...")
            tournament.generate_semifinals_round()
        else:
            print(f"    Generating Finals...")
            tournament.generate_unified_finals(after_semifinals=False)
...
def test_tournament_flow(team_count):
    ...
    try:
        ...
        # Note: Pod consistency algorithm groups same 4 teams together for multiple rounds,
        # which naturally leads to player-level repeat matchups. This is expected behavior.
        print_info("\nChecking for repeat matchups...")
        repeats = check_repeat_matchups(all_matchups)
        if repeats:
            # All team sizes may have repeat matchups due to pod consistency algorithm
            # The key constraint is that teammates never sit at the same table
            test_results['warnings'].append(f"Found {len(repeats)} repeat matchups (expected with pod consistency)")
            print_warning(f"Found {len(repeats)} repeat matchups (expected with pod consistency algorithm)")
        ...
        # Note: Playoffs are generated after Swiss results are submitted via simulate_round_results()
        if has_top8:
